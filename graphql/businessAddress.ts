...
export class BusinessAddress extends Model<BusinessAddressModel | null> {
  ...
  public async lastBusinessAddress(): Promise<BusinessAddressModel | null> {
    const result: Query = await this.client.rawQuery(
      GET_LAST_BUSINESS_ADDRESS_QUERY
    );
    return result.viewer?.lastBusinessAddress ?? null;
  }
   ...
   */
  public async create(
    args: MutationCreateBusinessAddressArgs
  ): Promise<BusinessAddressModel> {
    const result = await this.client.rawQuery(
      CREATE_BUSINESS_ADDRESS_MUTATION,
      args
    );
